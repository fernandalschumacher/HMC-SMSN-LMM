...
parameters {
		   ...
		   //
		   vector<lower=0, upper=1>[n] uvec;//mixing variable for slash
...
transformed parameters {
  ...
  //
  real ssl_const = sqrt(2.0 / pi()) * (nu / (nu - 0.5));
  //
  {
    matrix[q1,q1] dL = cholesky_decompose(Gamma);
	for (j in 1:n){
	  bvec[,j] = Delta * (-ssl_const + inv_sqrt(uvec[j]) * t0vec[j]) 
                   + inv_sqrt(uvec[j]) * dL * etavec[,j];
    }
  }
}
model {
  ...
  //
  uvec ~ beta(nu,1);
  nu ~ exponential(0.2); //mean 5, var 25
  t0vec ~ std_normal();
  {
	vector[N] yhat;
	int njloc = 0;
