---
title: "Supplementary Material: Bayesian estimation in scale mixture of skew-normal
  linear mixed models using Hamiltonian Monte Carlo"
...
We examine the convergence and distribution of the posterior samples.
```{r fit1converg}
# Extract draws for plotting
post_draws <- as_draws_array(tbayes) %>%
  subset_draws(variable = c("beta", "sigmae", "phi1", "nu"), regex = TRUE)

# Rank histograms (Convergence check)
mcmc_rank_hist(tbayes, pars=c("sigmae","phi1","nu"))
```
...
bayesplot::mcmc_dens_chains(tbayes, pars = c("beta[1]","beta[2]","phi1","lambda[1]","lambda[2]","nu")) +  theme_bw()
```
...
We compare the Skew-t model against a standard Normal model.
...
```{r fit1loo}
comp_loo <- loo_compare(list(Skew_T = st_loo$loo, Normal = normal_loo$loo))
knitr::kable(comp_loo, caption = "Model Comparison: LOO Criteria", simplify = FALSE)
```

### Comparing the models based on WAIC
```{r fit1waic}
comp_loo <- loo_compare(list(Skew_T = st_loo$waic, Normal = normal_loo$waic))
...
To evaluate the model's sensitivity to the choice of the prior distribution for nu, we now compare our original results based on a truncated Cauchy distribution with the ones considering a Gamma(2, 0.2) prior distribution for nu. 
```{r gammadist}
