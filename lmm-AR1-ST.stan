...
model {
  beta ~ normal(0,100);
  sigmae ~ student_t(4,0,5);//ou cauchy(0, 2.5);
  // 
  ddsqrt ~ student_t(4,0,5);//ou cauchy(0, 2.5);
  Lcorr ~ lkj_corr_cholesky(2.0); // prior for cholesky factor of a correlation matrix
  to_vector(etavec) ~ std_normal();
  //phi1 ~ normal(0, 2);//uniform(-1,1);
  nu ~ cauchy(0, 2.5);//ou student_t(4,0,5);
  //
  lambda ~ normal(0,sdLP);//normal(0,2.5);
  //
