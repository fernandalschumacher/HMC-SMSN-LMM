...
logveroARind = function(y, x, z, time, ind, beta1, sigmae, phiAR, D1, lambda, distr, nu){ #ind = indicadora de individuo
  ...
  if (distr=="sn") lv = (tapply(1:N,ind,ljnormalAR,y=y,x=x,z=z,time=time,beta1=beta1,Gammab=Gammab,Deltab=Deltab,sigmae=sigmae,phiAR=phiAR))
  else if (distr=="st") lv = (tapply(1:N,ind,ljtAR,nu=nu,y=y,x=x,z=z,time=time,beta1=beta1,Gammab=Gammab,Deltab=Deltab,sigmae=sigmae,phiAR=phiAR))
  else if (distr=="ss") lv = (tapply(1:N,ind,ljsAR,nu=nu,y=y,x=x,z=z,time=time,beta1=beta1,Gammab=Gammab,Deltab=Deltab,sigmae=sigmae,phiAR=phiAR))
  else if (distr=="scn") lv = (tapply(1:N,ind,ljcnAR,nu=nu,y=y,x=x,z=z,time=time,beta1=beta1,Gammab=Gammab,Deltab=Deltab,sigmae=sigmae,phiAR=phiAR))
  lv
}
#
...
################################################################
#Log-likelihood - AR(p) symmetric distributions
################################################################
...
logveroARsind = function(y,x,z,time,ind,beta1,sigmae,phiAR,D1,distr,nu){ #ind = indicadora de individuo
  N <-length(ind)
  
  if (distr=="sn") lv = (tapply(1:N,ind,ljnormalARs,y=y,x=x,z=z,time=time,beta1=beta1,D1=D1,sigmae=sigmae,phiAR=phiAR))
  else if (distr=="st") lv = (tapply(1:N,ind,ljtARs,nu=nu,y=y,x=x,z=z,time=time,beta1=beta1,D1=D1,sigmae=sigmae,phiAR=phiAR))
  else if (distr=="ss") lv = (tapply(1:N,ind,ljsARs,nu=nu,y=y,x=x,z=z,time=time,beta1=beta1,D1=D1,sigmae=sigmae,phiAR=phiAR))
  else if (distr=="scn") lv = (tapply(1:N,ind,ljcnARs,nu=nu,y=y,x=x,z=z,time=time,beta1=beta1,D1=D1,sigmae=sigmae,phiAR=phiAR))
  lv
}
...
criteriaAR1 <- function(stan_fit, data_list, distr = "st", progress_bar = interactive()) {
  ...
  phi1_chain   <- rstan::extract(stan_fit, pars = "phi1",   permuted = FALSE)
  D1_chain     <- rstan::extract(stan_fit, pars = "D1",     permuted = FALSE)
  #
  all_pars <- stan_fit@model_pars
  lambda_chain <- if ("lambda" %in% all_pars) {
    rstan::extract(stan_fit, pars = "lambda", permuted = FALSE)
  } else {
    NULL 
  }
  ...
  nchains <- dim(beta_chain)[2]
  n_subj  <- length(unique(data_list$indnum)) # Number of individuals
  
  # Compute Log-Likelihood Array [iter, chain, subject]
  ...
  for (i in seq_len(niter)) {
    for (j in seq_len(nchains)) {
      ...
      loglik_array[i, j, ] <- logveroARind(
        ...
        x      = data_list$x, 
        z      = data_list$z, 
        time   = data_list$timevar, 
        ind    = data_list$indnum, 
        beta1  = beta_chain[i, j, ], 
        sigmae = sigmae_chain[i, j, ], 
        phiAR  = phi1_chain[i, j, ], 
        D1     = D1_mat, 
        lambda = lambda_chain[i, j, ], 
        distr  = distr, 
        nu     = current_nu
      )
    }
  ...
  # Diagnostics: Relative Effective Sample Size
  # We use the separate chains here before they were flattened
  message("Calculating relative effective sample size...")
  r_eff <- relative_eff(exp(log_lik_matrix), 
                        chain_id = rep(1:nchains, each = niter))
  
  message("Computing LOO and WAIC...")
